# ARCHIVED - CS3 Research and Blueprint Plan

> **REFERENCE ONLY - DO NOT EXECUTE THIS PLAN.** This is the retired research
> plan and pre-cleanup continuation snapshot. R1-R3 and U1-U4 are already done.
> Every old launch gate, next-step instruction and status below is historical.
> The only active work plan is the repository-root `plan.md`; use the committed
> `docs/architecture/cs3-continuation-goal.md` for a new session.
...
**Execution state:** **Paused September 11, 2026 for a fresh-session handoff.**
U1-U4 are complete; U5 has a verified technical prototype but is not qualified;
U6-U10 remain pending. Current evidence and committed checkpoints are summarized in
[README.md](pre-handoff-readme.md) and [TODOS.md](../../TODOS.md).
...
at 18:26:14 EDT. The launch envelope and source/publication uses were subsequently
approved and recorded in `.artifacts/implementation/20260910T232859Z/authorization.json`.
Do not restart R1-R3 or repeat the satisfied launch gate.

**Original research goal:** Explain how CS1 and CS2 were built, recover the full Cel Shift art
...
The original research scope below produced a comparative explanation, a
reproducible technical probe and the blueprint. The later launch authorization,
not research approval alone, authorized implementation. Deployment remains
separately controlled.

## September 11 pause and continuation plan

The user requested a pause and a brand-new CS3 session on September 11, 2026
at 16:36:56 UTC. This session must not restart implementation or background
work. The pause does not renew the original allowance: heavy work ends
September 12, 2026 at 22:28:59 UTC and closeout ends at 23:28:59 UTC.

**Separate ownership:** The user reports that AR1, the production-ready Cel
Shift authoring/rendering milestone, is running in another session in
`williamsmat_microsoft/midcreek-concept`. CS3 must not duplicate its appearance
experiments, edit that sibling, or interfere with its processes.

**Continuation direction:** Use the current C5 candidate as the proposed
frozen development baseline while AR1 closes the visual-authoring gap.
This is not a declaration that U5 passed. Record any provisional-use exception
against the exact source/library/rendering-profile hashes and its permitted
development uses before applying it. Keep technical, appearance, performance,
publication and final human acceptance separate. See the
[blueprint continuation boundary](../architecture/cs3-blueprint.md#september-11-continuation-provisional-assets-and-ar1).

The next session should:

1. Verify the unchanged authorization, remaining allowance, checkout, C5 input
   hashes and absence of another CS3 integration owner. Resume only on the
   user's new-session instruction; do not treat this paused session as running.
2. Start from the committed C5 source checkpoint `9844435` and its retained
   source-review/validation evidence. Earlier handoffs describing C5 as
   uncommitted are historical; do not repeat its completed source correction.
3. Implement U6's guarded loading and resource lifetimes from its existing
   U1/U2 dependency. This implementation does not need to wait for AR1.
4. Finish U5's technical manifest/export/repetition/failure-path work and a
   clearly separated, explicitly authorized development-library path. Do not
   set appearance approval or replace a qualified production pointer to bypass
   an unmet gate.
5. Build U7's actual playable scene/HUD/animation, then U8 metrics, U9 showcase
   and U10 release controls. Qualification claims identify the exact frozen
   baseline; production release remains blocked wherever its gates are unmet.
6. Accept AR1 output only through a versioned, provenance-checked handoff and
   any required input/policy amendment. Rerun affected technical, integration,
   performance and publication checks after replacement; do not chase a moving
   sibling tip during qualification.

No further aesthetic sweeps are planned in this CS3 session. The immediate
next meaningful product result is a playable hall, not a larger image catalog.
If AR1 or release prerequisites remain unavailable at closeout, report the
implemented units and exact remaining gates rather than claiming full U10
or appearance completion.

The copyable [fresh-session goal](../architecture/cs3-continuation-goal.md)
travels with this committed handoff. Private references, generated candidate
media and operational receipts remain in their existing ignored locations;
they are not silently included in repository publication.
...
[front-loaded checkpoint](../architecture/cs3-blueprint.md#one-front-loaded-human-checkpoint)
...
The subsequent launch receipt contains the approved limits and a user-directed
AI-credit-ceiling waiver, not a waiver of deadlines, disk limits or new-service
restrictions. The original workflow amendment alone was not R3 approval.
...
| Launch preflight | Complete the remaining budget/access/source-use fields together before U1 | Satisfied September 10, 2026; exact authorization and credit amendment retained under the implementation run root |
| Current execution | Resume the paused CS3 work in a fresh session | Paused by user September 11, 2026; no automatic restart or allowance renewal |
...
complete at [docs/architecture/cs3-blueprint.md](../architecture/cs3-blueprint.md).
